...
export class StringHelper {
    ...
    public static romanToInt(roman: string): number {
        ...
        const valueMap: Record<RomanChar, number> = { I: 1, V: 5, X: 10, L: 50, C: 100, D: 500, M: 1000};
        ...
        if (this.intToRoman(total) !== normalized)
    ...
    public static initialsOf(text: string = '', length: number = 3): string {
        ...
        if (/\"|\\/.test(text) || text.length > 30 || (text = text?.replace?.(post, ' ').trim() ?? '').split(space).join('').length < 4)
            throw new Error('Create a different Name!');
        
        result = text.split(space).reduce((res, s) => res + s[0], '');

        if (result.length <= 1)
            result += text.replace(space, '')[1];
