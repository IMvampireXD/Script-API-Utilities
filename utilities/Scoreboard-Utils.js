...
export class CustomScoreboard {
	...
	addScore(player, score) {
		try {
			...
			console.warn(
				`${player} does not exist. Or the amount is not a allowed integer. Or the objective does not exist.`,
			);
		}
	}
	/**
...
export const getScore = (player, objective) => {
	return new CustomScoreboard(objective).getScore(player);
};
	...
	for (const player of world.getAllPlayers()) {
		...
		if (joined == undefined || joined !== player.name) {
			world.setDynamicProperty(
				`Name:${player.id}:${JSON.stringify(player.scoreboardIdentity)}`,
				player.name,
			);
		}
	}
