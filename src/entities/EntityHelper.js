...
*/
export function move(entity, targetPos, speed) {
...
export function isPlayerOnSurface(player) {
	...
	const blockBelow = player.dimension.getBlock(new Vec3(player.location.x, player.location.y, player.location.z).subtract({
		x: 0,
		y: 1,
		z: 0
	}));
	const blockAbove = player.dimension.getBlock(new Vec3(location.x, location.y, location.z).add({
		x: 0,
		y: 1,
		z: 0
	}));
